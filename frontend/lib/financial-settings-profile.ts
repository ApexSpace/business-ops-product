...
export function applyInvoiceDefaults(
  base: import("@/lib/invoice-profile").InvoiceFormValues,
  settings: FinancialSettingsResponse,
): import("@/lib/invoice-profile").InvoiceFormValues {
...
export function applyEstimateDefaults(
  base: import("@/lib/estimate-profile").EstimateFormValues,
  settings: FinancialSettingsResponse,
): import("@/lib/estimate-profile").EstimateFormValues {
  const issueDate = base.issueDate || new Date().toISOString().slice(0, 10);
  const expiryDate =
    base.expiryDate ||
