...
} from "@/components/calendars/calendar-edit-sections";
...
export function CalendarSettingsEditor({ calendarId }: CalendarSettingsEditorProps) {
  ...
  const { data: detail, isLoading } = useQuery({
    ...
    queryFn: () => apiClient<CalendarDetail>(`calendars/${calendarId}`),
  });
  ...
  const { data: googleResources } = useQuery({
    ...
    queryFn: () =>
      apiClient<IntegrationResourcesListResponse>(
        "integrations/business/google-calendar/resources",
      ),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: CalendarFormValues) => {
      ...
      await apiClient(`calendars/${calendarId}`, { method: "PATCH", body });
      await apiClient(`calendars/${calendarId}/availability`, {
        method: "PUT",
        body: { slots: availability },
      });
    },
    onSuccess: async () => {
