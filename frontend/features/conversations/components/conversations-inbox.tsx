...
export function ConversationsInbox() {
  ...
  useEffect(() => {
    if (selected?.channel === "EMAIL") {
      setEmailSubject(selected.title ?? "");
    } else {
      setEmailSubject("");
    }
  }, [selected?.channel, selected?.id, selected?.title]);
  ...
  return (
    <div className="flex h-[min(720px,calc(100svh-12rem))] min-h-[420px] overflow-hidden rounded-xl border border-border/80 bg-card shadow-sm">
