...
export function useOfferStaffPermissions() {
  ...
  const canAccess =
    isAdmin || hasStaffPermission(permissions, "offers.access", role);
  const canManage =
    isAdmin || hasStaffPermission(permissions, "offers.manage", role);
